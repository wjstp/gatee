...
HEALTHY_COUNT=$(docker-compose -p ${DOCKER_APP_NAME}-${AFTER_COMPOSE_COLOR} -f docker-compose.${AFTER_COMPOSE_COLOR}.yaml ps -q | xargs -I {} docker inspect --format '{{.Name}} {{if .State.Health}}{{.State.Health.Status}}{{end}}' {} | awk '/\/gatee-api-'${AFTER_COMPOSE_COLOR}'/ && /healthy/ {count++} END {print count+0}')

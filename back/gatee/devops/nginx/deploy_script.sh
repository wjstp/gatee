...
HEALTHY_COUNT=$(docker-compose -p ${DOCKER_APP_NAME}-${AFTER_COMPOSE_COLOR} -f docker-compose.${AFTER_COMPOSE_COLOR}.yaml ps -q | xargs -I {} docker inspect --format '{{if .State.Health}}{{.Name}} {{.State.Health.Status}}{{end}}' {} | grep "/gatee-api-${AFTER_COMPOSE_COLOR}" | grep -c "healthy" || echo "0")
